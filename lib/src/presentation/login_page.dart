...
class _LoginPageState extends State<LoginPage>{
  ...
  Future<void> go(bool signUp) async{
    ...
    try{
      if(signUp){
        ...
        if(r.session==null){setState(()=>message='Check your email to confirm your account, then sign in.');}
        else{widget.onSignedIn();}
      }else{
  ...
  @override Widget build(BuildContext context)=>Scaffold(body:Center(child:SingleChildScrollView(
    padding:const EdgeInsets.all(24),child:ConstrainedBox(constraints:const BoxConstraints(maxWidth:420),child:Card(
      child:Padding(padding:const EdgeInsets.all(28),child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        const SizedBox(height:16),const Text('MONTHLY BUDGET',style:TextStyle(fontSize:22,fontWeight:FontWeight.w800,letterSpacing:2)),
        const SizedBox(height:24),TextField(controller:email,keyboardType:TextInputType.emailAddress,decoration:const InputDecoration(labelText:'Email')),
        ...
        const SizedBox(height:20),SizedBox(width:double.infinity,child:FilledButton(onPressed:busy?null:()=>go(false),child:Text(busy?'Please wait...':'Sign in'))),
